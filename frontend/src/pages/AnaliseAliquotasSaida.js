...
const AnaliseAliquotasSaida = ({ user, onLogout }) => {
  ...
  const [filtroAlerta, setFiltroAlerta] = useState('todos'); // todos, com_alerta, sem_alerta
  ...
  const filteredProducts = analise?.produtos?.filter(p => {
  ...
  const exportToCSV = () => {
    if (!analise || !analise.produtos) return;
    
    const headers = 'Documento,Código,Descrição,Valor Total,ICMS %,PIS %,COFINS %,ICMS R$,PIS R$,COFINS R$,Alertas\n';
    const rows = analise.produtos.map(p => {
            ...
            <div className="bg-white rounded-xl p-4 shadow-md border border-gray-100">
              <div className="flex items-center justify-between flex-wrap gap-4">
                <div>
                  ...
                  <p className="text-sm text-gray-500">Competência: {analise.competencia}</p>
                </div>
                <button
                  onClick={exportToCSV}
            ...
            <div className="bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden">
              <div className="bg-gray-50 px-4 py-3 border-b border-gray-200 flex items-center justify-between flex-wrap gap-4">
                <h3 className="font-semibold text-gray-900">
                  Produtos Analisados ({filteredProducts.length})
                </h3>
                ...
                {filteredProducts.length === 0 ? (
                  ...
                  <table className="w-full text-sm">
                    <thead className="bg-gray-50 sticky top-0">
                      <tr>
                        <th className="px-4 py-2 text-left text-gray-600">NF-e</th>
                        <th className="px-4 py-2 text-left text-gray-600">Produto</th>
                        <th className="px-4 py-2 text-right text-gray-600">Valor</th>
                        <th className="px-4 py-2 text-center text-gray-600">ICMS</th>
                        <th className="px-4 py-2 text-center text-gray-600">PIS</th>
                        <th className="px-4 py-2 text-center text-gray-600">COFINS</th>
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {filteredProducts.map((produto, idx) => (
                        <tr key={idx} className={produto.alertas.length > 0 ? 'bg-orange-50/50' : ''}>
                          <td className="px-4 py-3 text-gray-900 font-medium">{produto.documento}</td>
                          <td className="px-4 py-3">
                            <p className="text-gray-900 font-medium truncate max-w-[200px]" title={produto.descricao}>
                              {produto.descricao}
                            </p>
                          ...
                          <td className="px-4 py-3 text-center">
                            <span className={`px-2 py-1 rounded text-xs font-medium ${
                              produto.aliquotas?.pis === 0 ? 'bg-gray-100 text-gray-600' : 'bg-purple-100 text-purple-800'
                            }`}>
                          ...
                          <td className="px-4 py-3 text-center">
                            <span className={`px-2 py-1 rounded text-xs font-medium ${
                              produto.aliquotas?.cofins === 0 ? 'bg-gray-100 text-gray-600' : 'bg-pink-100 text-pink-800'
                            }`}>
                          ...
                          <td className="px-4 py-3 text-center">
                            {produto.alertas.length === 0 ? (
                              ...
                              <div className="relative group">
                                ...
                                <div className="absolute right-0 top-6 z-10 hidden group-hover:block w-64 p-2 bg-white border rounded-lg shadow-lg">
                                  {produto.alertas.map((alerta, i) => (
                                    <div key={i} className={`p-2 mb-1 rounded text-xs ${getAlertColor(alerta.tipo)}`}>
            ...
            <div className="bg-blue-50 rounded-xl p-4 border border-blue-200">
              <div className="flex items-start gap-3">
                ...
                <div className="text-sm text-blue-700">
                  <p className="font-medium mb-1">Alíquotas Padrão Esperadas:</p>
                  <p>• ICMS: 18% (SP) | • PIS: 1,65% | • COFINS: 7,6%</p>
                  <p className="mt-1 text-blue-600">
                    Alíquotas zeradas podem ser corretas (isento, ST, alíquota zero, monofásico) - verifique caso a caso.
                  </p>
                </div>
              </div>
            </div>
