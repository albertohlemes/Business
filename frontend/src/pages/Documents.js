...
import { 
  ...
  Filter, CheckCircle, Ban, Receipt, Loader2, Clock, Calendar, FileArchive,
  Sparkles
} from 'lucide-react';
    ...
    return (
      <Layout user={user} onLogout={onLogout}>
        <div data-testid="documents-page" className="space-y-8">
          ...
          {/* Botões de Operação - Cards lado a lado com botões embaixo */}
          <div className="flex flex-col sm:flex-row items-start justify-center gap-6 py-8">
            ...
            <div className="flex flex-col items-center gap-3">
              ...
              <button
                ...
                className="w-64 flex items-center justify-center gap-2 px-4 py-2 text-sm text-red-400 bg-red-500/10 border border-red-500/20 rounded-lg hover:bg-red-500/20 disabled:opacity-50 transition-all"
              >
                {deletingAll === 'entrada' ? (
                  <>
            ...
            <div className="flex flex-col items-center gap-3">
              ...
              {/* Botão Verificar Notas Ausentes - abaixo do Apagar Saídas */}
              <button
                data-testid="btn-notas-ausentes"
                onClick={handleOpenNotasAusentes}
                className="w-64 flex items-center justify-center gap-2 px-4 py-2 text-sm text-yellow-400 bg-yellow-500/10 border border-yellow-500/20 rounded-lg hover:bg-yellow-500/20 transition-all"
              >
                <AlertTriangle className="w-4 h-4" />
          ...
          {loading ? (
            ...
            <div 
              ...
              className={`flex items-center justify-center h-full transition-all ${
                isDragging 
                  ? 'bg-[#C8A951]/10 border-2 border-dashed border-[#C8A951] rounded-xl' 
                  : ''
              }`}
            >
              <div className="text-center max-w-md mx-auto p-8">
                {isDragging ? (
                  <>
                    <div className="w-20 h-20 mx-auto mb-6 rounded-full bg-[#C8A951]/20 flex items-center justify-center animate-pulse">
                      <Upload className="w-10 h-10 text-[#C8A951]" />
                    </div>
                    <h3 className="text-xl font-medium text-[#C8A951] mb-2">Solte os arquivos aqui</h3>
                    <p className="text-[#A1A1AA]">XML, ZIP ou arraste múltiplos arquivos</p>
                  </>
                ) : (
                  <>
                    <div className={`w-20 h-20 mx-auto mb-6 rounded-full flex items-center justify-center ${
                      isEntrada ? 'bg-emerald-500/10' : 'bg-blue-500/10'
                    }`}>
                      <FileArchive className={`w-10 h-10 ${isEntrada ? 'text-emerald-400' : 'text-blue-400'}`} />
                    </div>
                    ...
                    <div 
                      onClick={() => fileInputRef.current?.click()}
                      className={`border-2 border-dashed rounded-xl p-8 cursor-pointer transition-all hover:bg-white/5 ${
                        isEntrada ? 'border-emerald-500/30 hover:border-emerald-500' : 'border-blue-500/30 hover:border-blue-500'
                      }`}
                    >
                      <div className="flex items-center justify-center gap-6 mb-4">
                        <div className="text-center">
                          <FileText className="w-8 h-8 text-[#A1A1AA] mx-auto mb-1" />
                          <span className="text-xs text-[#666]">XML</span>
                        </div>
                        <div className="text-center">
                          <FileArchive className="w-8 h-8 text-[#C8A951] mx-auto mb-1" />
                          <span className="text-xs text-[#C8A951]">ZIP</span>
                        </div>
                      </div>
                      <p className="text-sm text-[#666]">
                        Aceita arquivos .xml individuais ou .zip com múltiplos XMLs
                      </p>
                    </div>

                    <button
                      onClick={() => fileInputRef.current?.click()}
                      className={`mt-6 inline-flex items-center gap-2 px-6 py-3 text-white rounded-lg font-medium transition-all ${
                        isEntrada 
              ...
              <div className="p-4 border-b border-[#2A2A2A]">
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
                  ...
                  <div className="bg-emerald-500/10 rounded-lg p-4 text-center border border-emerald-500/20">
                    ...
                    <p className="text-sm text-emerald-400 mt-1">Importados</p>
                  </div>
                  {/* Total Ativo (o que aparece no dashboard) */}
                  <div className="bg-blue-500/10 rounded-lg p-4 text-center border border-blue-500/20">
                    <p className="text-3xl font-bold text-blue-400">
                      {(uploadResult.sucesso || uploadResult.processados?.length || 0) - (uploadResult.canceladas || 0) - (uploadResult.devolucoes || 0)}
                    </p>
                    <p className="text-sm text-blue-400 mt-1">Ativos</p>
                  </div>
