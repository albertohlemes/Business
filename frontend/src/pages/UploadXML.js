...
const UploadXML = ({ user, onLogout }) => {
  ...
  const FileItem = ({ file, index }) => (
    <div key={index} className="flex items-center gap-2 text-sm bg-[#141414] p-2 rounded">
      <FileText className="w-4 h-4 text-[#C8A951]" />
      <span className="text-[#E0E0E0] flex-1">{file.name}</span>
  ...
  const ProgressBar = () => (
    <div data-testid="upload-progress-bar" className="fixed top-0 left-0 right-0 z-50 bg-[#141414] shadow-lg border-b-2 border-[#C8A951]">
      <div className="max-w-5xl mx-auto px-4 py-4">
        ...
        {/* Info detalhada */}
        <div className="flex justify-between items-center mt-2 text-xs text-[#A1A1AA]">
          <span>
          ...
          {progress.currentFile && (
            <span className="truncate max-w-xs">
              Arquivo atual: <span className="font-medium text-[#E0E0E0]">{progress.currentFile}</span>
            </span>
          )}
        </div>
      </div>
  ...
  return (
    <Layout user={user} onLogout={onLogout}>
      ...
      <div data-testid="upload-xml-page" className={`space-y-6 max-w-5xl mx-auto ${(uploading || globalUploading) ? 'pt-32' : ''}`}>
        <div className="bg-gradient-to-r from-[#141414] to-orange-600 text-white rounded-lg p-6 shadow-lg">
          <div className="flex items-center gap-3 mb-2">
            <Sparkles className="w-8 h-8" />
            <h1 className="text-3xl font-bold">Upload com Análise Inteligente</h1>
          </div>
          <p className="text-[#A1A1AA]">A CONVERSÃO DE CFOP ACONTECE AUTOMATICAMENTE ao fazer o upload! Veja o relatório detalhado após enviar.</p>
        </div>
        ...
        <div className="bg-[#141414] rounded-lg p-6 shadow-md border border-[#2A2A2A]">
          <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <div className="relative">
                  <Calendar className="absolute left-3 top-3 w-5 h-5 text-[#666666]" />
                  <input
                    data-testid="competencia-input"
                    type="text"
                    value={competencia}
                    onChange={(e) => setCompetencia(e.target.value)}
                    className="w-full pl-11 pr-4 py-3 border border-[#333333] rounded-lg"
                    placeholder="01/2024"
                    maxLength="7"
                    disabled={uploading}
                  />
                </div>
                <p className="text-xs text-[#A1A1AA] mt-1">Formato: MM/AAAA (Ex: 01/2024)</p>
              </div>
            </div>
              ...
              <div className="flex gap-4 flex-wrap">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    disabled={uploading}
                  />
                  <span className="text-[#E0E0E0]">Entrada (Compras)</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    disabled={uploading}
                  />
                  <span className="text-[#E0E0E0]">Saída (Vendas)</span>
                </label>
              </div>
            ...
            <div>
              <label className="block text-sm font-medium text-[#E0E0E0] mb-2">Arquivos XML *</label>
              <div className={`border-2 border-dashed rounded-lg p-8 text-center transition-colors ${uploading ? 'border-[#2A2A2A] bg-[#0C0C0C]' : 'border-[#333333] hover:border-red-500'}`}>
                <Upload className={`w-12 h-12 mx-auto mb-4 ${uploading ? 'text-gray-300' : 'text-[#666666]'}`} />
                <input
                  id="file-input"
                ...
                <label htmlFor="file-input" className={uploading ? 'cursor-not-allowed' : 'cursor-pointer'}>
                  <span className={`font-semibold ${uploading ? 'text-[#666666]' : 'text-[#C8A951] hover:text-red-700'}`}>
                    {uploading ? 'Upload em andamento...' : 'Clique para selecionar'}
                  </span>
                  {!uploading && <span className="text-[#A1A1AA]"> ou arraste os arquivos aqui</span>}
                </label>
            ...
            <button
              ...
              className="w-full bg-[#C8A951] text-white py-4 rounded-lg font-bold hover:bg-[#B09240] disabled:opacity-50 disabled:cursor-not-allowed shadow-lg text-lg flex items-center justify-center gap-2"
            >
              {uploading || globalUploading ? (
                <>
        ...
        {results && (
          <div className="bg-[#141414] rounded-lg p-6 shadow-md border border-[#2A2A2A]">
            <h2 className="text-xl font-bold text-white mb-4">Resultado do Upload</h2>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
              ...
              <div className="bg-amber-500/10 p-4 rounded-lg border border-amber-500/20 text-center">
                <p className="text-2xl font-bold text-amber-400">{results.resumo?.duplicados || results.duplicadas?.length || 0}</p>
              ...
              <div className="bg-red-50 p-4 rounded-lg border border-red-200 text-center">
                ...
                <p className="text-sm text-[#C8A951]">Erros</p>
              </div>
            ...
            {results.duplicadas && results.duplicadas.length > 0 && (
              <div className="mb-4">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h3 className="font-semibold text-amber-400">Notas Duplicadas ({results.duplicadas.length})</h3>
                </div>
                <div className="space-y-2 max-h-40 overflow-y-auto">
                  {results.duplicadas.slice(0, 10).map((item, index) => (
                    <div key={index} className="bg-amber-500/10 p-3 rounded-lg border border-amber-500/20">
                      <p className="text-sm font-medium text-amber-400">{typeof item === 'string' ? item : (item.filename || item.numero_nfe || 'Duplicada')}</p>
                    </div>
                  ))}
            ...
            {results.errors && results.errors.length > 0 && (
              <div className="mb-4">
                <div className="flex items-center gap-2 mb-3">
                  <AlertCircle className="w-5 h-5 text-[#C8A951]" />
                ...
                <div className="space-y-2">
                  {results.rejeitadas_cnpj.map((item, index) => (
                    <div key={index} className="bg-red-50 p-3 rounded-lg border border-red-200">
                      ...
                      <p className="text-xs text-[#A1A1AA] mt-1">Emitente: {item.emitente} | Destinatário: {item.destinatario}</p>
                    </div>
                  ))}
                </div>
              </div>
                ...
                <div className="space-y-2">
                  {results.rejeitadas_competencia.map((item, index) => (
                    <div key={index} className="bg-orange-50 p-3 rounded-lg border border-orange-200">
                      ...
                      <p className="text-xs text-[#A1A1AA] mt-1">Data de Emissão: {item.data_emissao}</p>
                    </div>
                  ))}
                </div>
              </div>
                    ...
                    <div key={idx} className="bg-amber-50 p-4 rounded-lg border-2 border-amber-300">
                      ...
                      <div className="space-y-2">
                        {arquivo.alertas.map((alerta, i) => (
                          <div key={i} className="bg-[#141414] p-2 rounded border border-amber-200 flex items-center justify-between flex-wrap gap-2">
                            <div>
                              <p className="text-sm font-medium text-white">{alerta.produto}</p>
                              <p className="text-xs text-[#A1A1AA]">Código: {alerta.codigo}</p>
                            </div>
                ...
                <div className="space-y-3 max-h-64 overflow-y-auto">
                  {results.notas_desconsideradas_devolucao.map((nota, idx) => (
                    <div key={idx} className={`p-4 rounded-lg border-2 ${
                      ...
                      <p className="text-sm text-[#E0E0E0]">{nota.motivo}</p>
                      {nota.emitente && (
                        <p className="text-xs text-[#A1A1AA] mt-1">Emitente: {nota.emitente}</p>
                      )}
                      {nota.destinatario && (
                        <p className="text-xs text-[#A1A1AA]">Destinatário: {nota.destinatario}</p>
                      )}
            ...
            {results.resumo && (
              <div className="mb-4 bg-[#0C0C0C] rounded-lg p-4 border border-[#2A2A2A]">
                <h3 className="font-bold text-white mb-3">Resumo da Importação</h3>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 text-center">
                  <div className="bg-[#141414] p-3 rounded-lg">
                    <p className="text-2xl font-bold text-white">{results.resumo.total_arquivos}</p>
                    <p className="text-xs text-[#A1A1AA]">Total Enviados</p>
                  </div>
                  ...
                  <div className="bg-yellow-100 p-3 rounded-lg">
                    <p className="text-2xl font-bold text-amber-400">{results.resumo.duplicados}</p>
                  ...
                  <div className="bg-red-100 p-3 rounded-lg">
                    ...
                    <p className="text-xs text-[#C8A951]">CNPJ Errado</p>
                  </div>
                  ...
                  <div className="bg-[#2A2A2A] p-3 rounded-lg">
                    <p className="text-2xl font-bold text-[#E0E0E0]">{results.resumo.erros}</p>
                  ...
                  {results.performance && (
                    <div className="flex gap-4 mt-2 text-sm">
                      ...
                      {results.performance.produtos_da_ia > 0 && (
                        <span className="bg-amber-500/100 text-amber-400 px-2 py-1 rounded">
                    ...
                    <div key={idx} className="bg-purple-50 p-4 rounded-lg border-2 border-purple-300">
                      ...
                      <div className="space-y-3">
                        {arquivo.conversoes.map((conv, i) => (
                          <div key={i} className="bg-[#141414] p-3 rounded-lg border border-purple-200">
                            <div className="flex items-center justify-between mb-2">
                              <p className="font-semibold text-white">{conv.produto}</p>
                              {/* Origem da classificação */}
                              {conv.origem && (
                                <span className={`text-xs px-2 py-0.5 rounded-full ${
                                  conv.origem === 'cache' ? 'bg-green-100 text-green-700' :
                                  conv.origem === 'regra' ? 'bg-blue-100 text-blue-700' :
                                  'bg-yellow-100 text-amber-400'
                                }`}>
                            ...
                            <div className="flex items-center gap-3 flex-wrap">
                              <div className="flex items-center gap-2">
                                <span className="text-xs text-[#A1A1AA]">Original:</span>
                              ...
                              <div className="flex items-center gap-2">
                                <span className="text-xs text-[#A1A1AA]">Convertido:</span>
                            ...
                            <p className="text-xs text-[#A1A1AA] mt-2 font-medium">Critério: {conv.motivo}</p>
                          </div>
                        ))}
                      </div>
                    </div>
