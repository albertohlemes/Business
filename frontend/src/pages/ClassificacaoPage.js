    ...
    <span className="text-xs">
      {visibleNFs.map((o, idx) => (
        <span key={idx}>
          ...
          {idx < visibleNFs.length - 1 && <span className="text-[#666666]">, </span>}
        </span>
      ))}
      {hasMore && (
        <button
      ...
      {expanded && ocorrencias.length > maxVisible && (
        <button
          ...
          className="ml-1 text-[#A1A1AA] hover:text-[#E0E0E0]"
        >
          (ocultar)
        </button>
      )}
...
const ClassificacaoPage = ({ user, onLogout }) => {
  ...
  const productsByCategoria = useMemo(() => {
    const grupos = {
      ...
      'COMBUSTIVEL': { produtos: [], cor: 'bg-gray-700', corFundo: 'bg-[#0C0C0C]', icon: '⛽' },
  ...
  const getCategoryBadge = (categoria, natureza) => {
    ...
    if (!badge) {
      badge = { bg: 'bg-[#1A1A1A]', text: 'text-white', label: categoria?.toUpperCase() || 'N/A' };
    }
  ...
  // Componente ProductRow para visualização agrupada - convertido para função render
      ...
      <div className={`px-6 py-3 border-b border-[#2A2A2A] hover:bg-[#0C0C0C] flex items-center justify-between ${allApproved ? 'bg-emerald-500/10/50' : ''}`}>
          ...
          <button
            ...
            className={`w-6 h-6 rounded border-2 flex items-center justify-center transition-colors flex-shrink-0 ${
              allApproved 
                ...
                : 'border-[#333333] hover:border-green-500'
            }`}
          >
            {allApproved && <Check className="w-4 h-4" />}
          </button>
          <div className="min-w-0 flex-1">
            <p className="font-medium text-white truncate">{product.descricao}</p>
            <p className="text-xs text-[#A1A1AA]">
              Código: {product.codigo} • NCM: <span className="font-mono">{product.ncm}</span> • CFOP: {product.cfop}
            </p>
            <p className="text-xs">
              <span className="text-[#A1A1AA]">NFs: </span>
        ...
        <div className="flex items-center gap-4 ml-4">
          <div className="text-right">
            ...
            <p className="text-xs text-[#A1A1AA]">{product.ocorrencias.length} NF(s)</p>
          </div>
          <button
    ...
    return (
      <div key={product.codigo} className={`grid grid-cols-12 gap-2 items-center px-4 py-3 border-b hover:bg-[#0C0C0C] transition-colors ${allApproved ? 'bg-emerald-500/10' : ''}`}>
        ...
        <div className="col-span-1 flex items-center justify-center gap-2">
          <button
            ...
            className={`w-5 h-5 rounded border-2 flex items-center justify-center flex-shrink-0 ${
              isSelected ? 'bg-blue-600 border-blue-600 text-white' : 'border-[#333333] hover:border-blue-500'
            }`}
          >
          ...
          >
            {allApproved ? <Check className="w-4 h-4" /> : approved > 0 ? <span className="text-xs font-bold text-amber-400">{approved}</span> : null}
          </button>
        </div>
        
        {/* Produto */}
        <div className="col-span-3">
          <p className="font-semibold text-white text-sm truncate" title={product.descricao}>
            {product.descricao}
          </p>
          <p className="text-xs text-[#A1A1AA]">
            Cód: {product.codigo}
        ...
        <div className="col-span-1">
          <span className="font-mono text-xs bg-[#1A1A1A] px-1.5 py-0.5 rounded">
            {product.ncm || '-'}
          </span>
        </div>
        ...
        {/* CFOP */}
        <div className="col-span-1">
          <span className="px-2 py-1 rounded font-mono text-sm font-semibold bg-[#1A1A1A] text-white">
            {product.cfop}
          </span>
        </div>
  ...
  return (
    <Layout user={user} onLogout={onLogout}>
      <div data-testid="classificacao-page" className="space-y-6">
        ...
        {/* Header */}
        <div className="bg-gradient-to-r from-purple-600 to-red-600 text-white rounded-lg p-6 shadow-lg">
          <div className="flex items-center justify-between flex-wrap gap-4">
            ...
            <div className="flex gap-4">
              <div className="bg-[#141414]/20 px-4 py-2 rounded-lg text-center">
        ...
        {/* Ações em Lote */}
        <div className="bg-[#141414] rounded-lg p-4 border border-[#2A2A2A] shadow-sm">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <RefreshCw className="w-5 h-5 text-[#A1A1AA]" />
              <div>
                <h3 className="font-semibold text-white">Reimportar Entradas</h3>
                <p className="text-sm text-[#A1A1AA]">Reprocessa e classifica apenas notas de ENTRADA (saídas são ignoradas)</p>
              </div>
            </div>
            <button
              data-testid="btn-reimportar"
          ...
          {reimportProgress && (
            <div className="mt-4 pt-4 border-t border-[#2A2A2A]">
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm font-medium text-[#E0E0E0]">{reimportProgress.step}</span>
        ...
        <div className="bg-gradient-to-r from-purple-50 to-pink-50 rounded-lg p-4 border-2 border-purple-200">
          ...
          <div className="flex gap-2 mb-3 flex-wrap">
            <button 
              ...
              className="text-xs px-2 py-1 bg-[#141414] text-[#E0E0E0] rounded hover:bg-[#1A1A1A] border"
            >
              limpeza → DESPESA
            </button>
            <button 
          ...
          <div className="bg-purple-50 rounded-lg p-4 border border-purple-200">
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="px-3 py-1 bg-[#C8A951] text-white rounded-lg text-sm font-medium flex items-center gap-1 hover:bg-[#B09240]"
                >
                  <Trash2 className="w-4 h-4" />
                  Limpar Tudo
                </button>
            ...
            <div className="space-y-2 max-h-64 overflow-y-auto">
              {learnedRules.map((rule) => (
                <div key={rule.id} className="bg-[#141414] p-3 rounded-lg border border-purple-100 flex items-center gap-3">
                  {/* Checkbox */}
                  <input
                    type="checkbox"
                  ...
                  {editingRule === rule.id ? (
                    ...
                    <>
                      <div className="text-sm flex-1">
                        ...
                        <span className="text-[#A1A1AA] ml-2 font-mono">CFOP {rule.cfop_correto || rule.cfop}</span>
                      </div>
        ...
        {/* Controles */}
        {selectedCompany && (
          <div className="bg-[#141414] rounded-lg p-4 shadow-md border border-[#2A2A2A] flex items-center justify-between flex-wrap gap-4">
            <div>
              <p className="text-sm text-[#A1A1AA]">Validando entradas de</p>
              <p className="font-bold text-white">{selectedCompany.razao_social}</p>
        ...
        <div className="bg-[#141414] rounded-lg shadow-md border border-[#2A2A2A] overflow-hidden">
          <div className="bg-[#0C0C0C] px-4 py-3 border-b border-[#2A2A2A]">
                  ...
                  <button
                    ...
                    className={`px-3 py-1 rounded text-sm font-medium transition-colors ${
                      displayMode === 'agrupado' 
                        ? 'bg-[#141414] text-purple-700 shadow-sm' 
          ...
          {/* Campo de Busca */}
          <div className="px-4 py-3 bg-[#0C0C0C] border-b border-[#2A2A2A]">
            <div className="relative">
              <Search className="w-5 h-5 absolute left-3 top-1/2 -translate-y-1/2 text-[#666666]" />
              <input
                type="text"
                ...
                className="w-full pl-10 pr-10 py-2 border border-[#333333] rounded-lg text-sm focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
              />
              {searchTerm && (
                <button 
                  onClick={() => setSearchTerm('')}
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-[#666666] hover:text-[#A1A1AA]"
                >
                  <X className="w-4 h-4" />
                </button>
              )}
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div className="text-left">
                          <p className="font-bold text-white">{categoria.replace('_', ' ')}</p>
                          <p className="text-sm text-[#A1A1AA]">{produtos.length} produto(s) • {produtos.reduce((s, p) => s + p.ocorrencias.length, 0)} ocorrência(s)</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-4">
                        <div className="text-right">
                          <p className="text-sm text-[#A1A1AA]">Valor Total</p>
                          <p className="font-bold text-white">
          ...
          {displayMode === 'lista' && (
            <>
              {/* Cabeçalho ordenável */}
              <div className="bg-[#1A1A1A] px-4 py-2 border-b border-[#2A2A2A] grid grid-cols-12 gap-2 items-center text-sm font-semibold text-[#E0E0E0]">
      ...
      {reclassifyingProduct && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-[#141414] rounded-lg shadow-2xl max-w-lg w-full mx-4 overflow-hidden">
            ...
            <div className="p-6 space-y-4">
              <div className="bg-[#0C0C0C] rounded-lg p-4">
                <p className="font-semibold text-white">{reclassifyingProduct.product.descricao}</p>
              ...
              <div>
                <label className="block text-sm font-semibold text-[#E0E0E0] mb-2">Nova Categoria</label>
                <div className="grid grid-cols-2 gap-3">
                  {[
                    ...
                    >
                      <span className={`font-bold text-sm ${newCategoria === cat.id ? 'text-purple-700' : 'text-white'}`}>
                        {cat.label}
                      </span>
              ...
              <div>
                <label className="block text-sm font-semibold text-[#E0E0E0] mb-2">Justificativa (opcional)</label>
                <textarea
                  value={reclassifyMotivo}
                  onChange={(e) => setReclassifyMotivo(e.target.value)}
                  className="w-full px-4 py-2 border border-[#333333] rounded-lg text-sm"
                  rows="2"
            ...
            <div className="px-6 py-4 bg-[#0C0C0C] flex gap-3 justify-end">
              <button
                onClick={() => {
                  setReclassifyingProduct(null);
                  setNewCategoria('');
                ...
                className="px-4 py-2 bg-[#2A2A2A] text-[#E0E0E0] rounded-lg font-medium hover:bg-gray-300"
              >
                Cancelar
              </button>
              <button
