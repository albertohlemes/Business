...
const AnaliseTributariaIA = ({ user, onLogout }) => {
  ...
  const SortIcon = ({ columnKey }) => {
    if (sortConfig.key !== columnKey) {
      return <ArrowUpDown className="w-4 h-4 text-[#666666]" />;
    }
      ...
      <Layout user={user} onLogout={onLogout}>
        <div className="flex items-center justify-center h-96">
          <div className="text-center">
            ...
            <h2 className="text-xl font-semibold text-[#E0E0E0]">Selecione uma Empresa</h2>
            <p className="text-[#A1A1AA] mt-2">Clique no seletor no header para escolher uma empresa</p>
          </div>
        </div>
      </Layout>
    );
        ...
        {loading && !data && (
          <div className="flex items-center justify-center h-64">
            <div className="text-center">
              ...
              <p className="text-[#A1A1AA]">Analisando dados tributários com IA...</p>
            </div>
          </div>
        )}
          ...
          <>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="bg-[#141414] rounded-lg p-5 shadow-sm border border-[#2A2A2A]">
                ...
                <p className="text-2xl font-bold text-white">{data.resumo?.total_documentos || 0}</p>
              ...
              <div className="bg-[#141414] rounded-lg p-5 shadow-sm border border-[#2A2A2A]">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <span className="text-sm text-[#A1A1AA]">Crédito ICMS</span>
                </div>
                <p className="text-2xl font-bold text-green-600">{formatCurrency(data.resumo?.credito_icms)}</p>
                <p className="text-xs text-[#A1A1AA] mt-1">Total de entradas</p>
              </div>
              ...
              <div className="bg-[#141414] rounded-lg p-5 shadow-sm border border-[#2A2A2A]">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <span className="text-sm text-[#A1A1AA]">Débito ICMS</span>
                </div>
                <p className="text-2xl font-bold text-[#C8A951]">{formatCurrency(data.resumo?.debito_icms)}</p>
                <p className="text-xs text-[#A1A1AA] mt-1">Total de saídas</p>
              </div>

              <div className={`rounded-lg p-5 shadow-sm border ${data.resumo?.saldo_icms > 0 ? 'bg-amber-500/10 border-amber-500/20' : 'bg-blue-50 border-blue-200'}`}>
                <div className="flex items-center gap-3 mb-2">
                  <div className={`p-2 rounded-lg ${data.resumo?.saldo_icms > 0 ? 'bg-yellow-200' : 'bg-blue-200'}`}>
                    <DollarSign className={`w-5 h-5 ${data.resumo?.saldo_icms > 0 ? 'text-amber-400' : 'text-blue-700'}`} />
                  </div>
                  <span className="text-sm text-[#A1A1AA]">Saldo ICMS</span>
                </div>
                <p className={`text-2xl font-bold ${data.resumo?.saldo_icms > 0 ? 'text-amber-400' : 'text-blue-700'}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {/* Vilões */}
              <div className="bg-gradient-to-r from-red-500 to-orange-500 rounded-lg p-5 text-white">
                <div className="flex items-center gap-3 mb-3">
                  <AlertTriangle className="w-8 h-8" />
                  <div>
                    <h3 className="font-bold text-lg">Vilões Tributários</h3>
                    <p className="text-[#A1A1AA] text-sm">Produtos com prejuízo tributário</p>
                  </div>
                </div>
                <div className="flex items-end justify-between">
                  <div>
                    <p className="text-4xl font-bold">{data.resumo?.total_viloes || 0}</p>
                    <p className="text-[#A1A1AA] text-sm">produtos identificados</p>
                  </div>
                  <div className="text-right">
                    <p className="text-2xl font-bold">{formatCurrency(data.resumo?.impacto_viloes)}</p>
                    <p className="text-[#A1A1AA] text-sm">impacto negativo</p>
                  </div>
                </div>
              </div>
            ...
            {/* Filtros */}
            <div className="bg-[#141414] rounded-lg shadow-sm border border-[#2A2A2A] p-4">
              <div className="flex flex-wrap items-center gap-4">
                <div className="flex items-center gap-2">
                  <Filter className="w-5 h-5 text-[#A1A1AA]" />
                  <span className="font-medium text-[#E0E0E0]">Filtros:</span>
                </div>
                ...
                <div className="flex-1 min-w-[200px] max-w-[250px]">
                  <select
                    ...
                    className="w-full px-3 py-2 border border-[#333333] rounded-lg text-sm focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
                  >
                ...
                <div className="flex-1 min-w-[200px] max-w-[300px] relative">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-3 py-2 border border-[#333333] rounded-lg text-sm focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
                  />
                </div>
                ...
                {(filtroNCM || filtroProduto || sortConfig.key) && (
                  <button
                    ...
                    className="px-3 py-2 text-sm text-[#A1A1AA] hover:text-white hover:bg-[#1A1A1A] rounded-lg"
                  >
                    Limpar filtros
                  </button>
                )}
            ...
            {/* Tabs */}
            <div className="bg-[#141414] rounded-lg shadow-sm border border-[#2A2A2A]">
                ...
                <nav className="flex -mb-px">
                  <button
                    ...
                    className={`px-6 py-4 text-sm font-medium border-b-2 ${
                      activeTab === 'viloes'
                        ? 'border-red-500 text-[#C8A951]'
                        : 'border-transparent text-[#A1A1AA] hover:text-[#E0E0E0]'
                    }`}
                  >
                  ...
                  <div className="space-y-4">
                    {filteredViloes.length === 0 ? (
                      <div className="text-center py-8">
                        ...
                        <p className="text-[#A1A1AA]">Nenhum vilão tributário identificado!</p>
                        <p className="text-[#A1A1AA] text-sm">Sua tributação está equilibrada.</p>
                      </div>
                    ) : (
                      <>
                        {/* Cabeçalho ordenável */}
                        <div className="hidden md:grid grid-cols-6 gap-4 px-4 py-2 bg-[#0C0C0C] rounded-lg text-sm font-medium text-[#A1A1AA]">
                        ...
                        {filteredViloes.map((vilao, idx) => (
                          <div key={idx} className="border border-red-200 rounded-lg overflow-hidden">
                            ...
                            >
                              <div className="flex items-center gap-3">
                                ...
                                <div className="text-left">
                                  <p className="font-semibold text-white">{vilao.descricao}</p>
                                  <p className="text-sm text-[#A1A1AA]">NCM: {vilao.ncm}</p>
                                </div>
                              </div>
                              <div className="flex items-center gap-4">
                                <div className="hidden md:flex items-center gap-4 text-sm">
                                  <span className="text-[#A1A1AA]">{vilao.aliq_entrada}% → {vilao.aliq_saida}%</span>
                                  <span className="text-[#C8A951] font-medium">+{vilao.diferenca_aliquota}%</span>
                                </div>
                                <div className="text-right">
                                  <p className="font-bold text-[#C8A951]">{formatCurrency(vilao.impacto_negativo)}</p>
                                  <p className="text-xs text-[#A1A1AA]">impacto negativo</p>
                                </div>
                            ...
                            {expandedVilao === idx && (
                              <div className="p-4 bg-[#141414] border-t border-red-200">
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                                  <div className="bg-[#0C0C0C] p-3 rounded">
                                    <p className="text-xs text-[#A1A1AA]">Alíquota Entrada</p>
                                    <p className="font-bold text-white">{vilao.aliq_entrada}%</p>
                                  </div>
                                  <div className="bg-[#0C0C0C] p-3 rounded">
                                    <p className="text-xs text-[#A1A1AA]">Alíquota Saída</p>
                                    <p className="font-bold text-white">{vilao.aliq_saida}%</p>
                                  </div>
                                  <div className="bg-green-50 p-3 rounded">
                                    <p className="text-xs text-[#A1A1AA]">Crédito ICMS</p>
                                    <p className="font-bold text-green-600">{formatCurrency(vilao.icms_credito)}</p>
                                  </div>
                                  <div className="bg-red-50 p-3 rounded">
                                    <p className="text-xs text-[#A1A1AA]">Débito ICMS</p>
                                    <p className="font-bold text-[#C8A951]">{formatCurrency(vilao.icms_debito)}</p>
                                  </div>
                                </div>
                                ...
                                <div className="mt-3 flex gap-4 text-sm text-[#A1A1AA]">
                  ...
                  <div className="space-y-4">
                    {filteredOportunidades.length === 0 ? (
                      <div className="text-center py-8">
                        ...
                        <p className="text-[#A1A1AA]">Nenhuma oportunidade identificada</p>
                        <p className="text-[#A1A1AA] text-sm">Continue monitorando para identificar benefícios.</p>
                      </div>
                    ) : (
                        ...
                        <div key={idx} className="border border-green-200 rounded-lg overflow-hidden">
                          ...
                          >
                            <div className="flex items-center gap-3">
                              ...
                              <div className="text-left">
                                <p className="font-semibold text-white">{op.descricao}</p>
                                <p className="text-sm text-[#A1A1AA]">NCM: {op.ncm} • Tipo: {op.tipo?.replace(/_/g, ' ')}</p>
                              </div>
                            </div>
                            <div className="flex items-center gap-4">
                              <div className="text-right">
                                <p className="font-bold text-green-600">{formatCurrency(op.beneficio)}</p>
                                <p className="text-xs text-[#A1A1AA]">benefício</p>
                              </div>
                          ...
                          {expandedOportunidade === idx && (
                            <div className="p-4 bg-[#141414] border-t border-green-200">
                              ...
                              <div className="grid grid-cols-2 gap-4">
                                ...
                                <div className="bg-[#0C0C0C] p-3 rounded">
                                  <p className="text-xs text-[#A1A1AA]">Débito ICMS</p>
                                  <p className="font-bold text-[#A1A1AA]">{formatCurrency(op.icms_debito)}</p>
                                </div>
                              </div>
                            </div>
                          )}
                ...
                {activeTab === 'ncm' && (
                  <div className="overflow-x-auto">
                      ...
                      <thead className="bg-[#0C0C0C]">
                        <tr>
                          <th className="px-4 py-3 text-left">
                            <button onClick={() => handleSort('ncm')} className="flex items-center gap-1 text-xs font-semibold text-[#A1A1AA]">
                              NCM / Produto <SortIcon columnKey="ncm" />
                            </button>
                          </th>
                          <th className="px-4 py-3 text-center">
                            <button onClick={() => handleSort('aliq_entrada')} className="flex items-center gap-1 justify-center text-xs font-semibold text-[#A1A1AA] w-full">
                              Alíq. Entr. <SortIcon columnKey="aliq_entrada" />
                            </button>
                          </th>
                          <th className="px-4 py-3 text-center">
                            <button onClick={() => handleSort('aliq_saida')} className="flex items-center gap-1 justify-center text-xs font-semibold text-[#A1A1AA] w-full">
                              Alíq. Saída <SortIcon columnKey="aliq_saida" />
                            </button>
                          </th>
                          <th className="px-4 py-3 text-right">
                            <button onClick={() => handleSort('entrada_valor')} className="flex items-center gap-1 justify-end text-xs font-semibold text-[#A1A1AA] w-full">
                              Entrada (R$) <SortIcon columnKey="entrada_valor" />
                            </button>
                          </th>
                          <th className="px-4 py-3 text-right">
                            <button onClick={() => handleSort('entrada_icms_creditavel')} className="flex items-center gap-1 justify-end text-xs font-semibold text-[#A1A1AA] w-full">
                              Créd. ICMS <SortIcon columnKey="entrada_icms_creditavel" />
                            </button>
                          </th>
                          <th className="px-4 py-3 text-right">
                            <button onClick={() => handleSort('saida_valor')} className="flex items-center gap-1 justify-end text-xs font-semibold text-[#A1A1AA] w-full">
                              Saída (R$) <SortIcon columnKey="saida_valor" />
                            </button>
                          </th>
                          <th className="px-4 py-3 text-right">
                            <button onClick={() => handleSort('saida_icms')} className="flex items-center gap-1 justify-end text-xs font-semibold text-[#A1A1AA] w-full">
                              Déb. ICMS <SortIcon columnKey="saida_icms" />
                            </button>
                          </th>
                          <th className="px-4 py-3 text-right">
                            <button onClick={() => handleSort('saldo_icms')} className="flex items-center gap-1 justify-end text-xs font-semibold text-[#A1A1AA] w-full">
                          ...
                          return (
                            ...
                            >
                              <td className="px-4 py-3">
                                ...
                                {tituloProduto && (
                                  <div className="text-xs text-[#A1A1AA] mt-0.5">{tituloProduto}</div>
                                )}
                              </td>
                              ...
                              <td className={`px-4 py-3 text-sm text-center ${temDiferencaAliquota && ncm.aliq_saida > ncm.aliq_entrada ? 'text-[#C8A951] font-bold' : ''}`}>
                              ...
                              <td className="px-4 py-3 text-sm text-right text-[#C8A951]">{formatCurrency(ncm.saida_icms)}</td>
                              <td className={`px-4 py-3 text-sm text-right font-semibold ${ncm.saldo_icms > 0 ? 'text-[#C8A951]' : 'text-green-600'}`}>
                    ...
                    {filteredNcmData.length === 0 && (
                      <div className="text-center py-8">
                        <Package className="w-12 h-12 text-[#666666] mx-auto mb-3" />
                        <p className="text-[#A1A1AA]">Nenhum NCM encontrado</p>
                      </div>
                    )}
                  </div>
                )}
                ...
                {activeTab === 'insights' && (
                  <div className="prose max-w-none">
                    {data.insights_ia ? (
                        ...
                        <div className="text-white leading-relaxed space-y-4">
                          {data.insights_ia.split('\n').map((line, idx) => {
                            ...
                            if (isTitulo) {
                              ...
                              return <p key={idx} className="text-[#E0E0E0]">{cleanLine}</p>;
                            }
                          })}
                        </div>
                      </div>
                      ...
                      <div className="text-center py-8">
                        <Brain className="w-12 h-12 text-[#666666] mx-auto mb-3" />
                        <p className="text-[#A1A1AA]">Insights não disponíveis</p>
                        <p className="text-[#A1A1AA] text-sm">Clique em &quot;Atualizar Análise&quot; para gerar insights com IA.</p>
                      </div>
                    )}
                  </div>
                )}
